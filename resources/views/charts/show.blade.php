    ...
    @if($chart->isActive())
        ...
        @if($voted)
            <div class="alert alert-success">
                <p>Du hast bereits für diese Charts abgestimmt! Hier siehst du die ergebnisse für deine Votes</p>
            </div>
            <table id="Table" class="table table-striped" style="width: 100%;" data-toggle="table"
                ...
                @foreach($chart->getUserVotedSong() as $key => $value)
                    <tr>
                        ...
                        <td style="width: 15%"><a class="btn btn-primary" target="_blank"
                        ...
                        @forelse($chart->getSongs() as $song)
                            <tr>
                                ...
                                <td style="width: 15%"><a class="btn btn-primary"
                                                          href="{{route('song.show', $song->id)}}">Anhören</a>
                                </td>
                                @auth
                                    <td><input class="form-control" name="vote/{{$song->id}}" type="number" min="0"
                                               max="3"
                                               placeholder="Platzierung von 1 bis 3"
                                               value="{{old("vote/$song->id")??""}}">
                                    </td>
                                @endauth
                            </tr>
                        @empty
    ...
    @else
        ...
        @else
            ...
            <div class="alert alert-danger">
                <p>Die Abstimmung kann am {!! date('d.m.Y',strtotime($chart->show_date)) !!} um {!! date('H:i',strtotime($chart->show_date)) !!} angeschauen werden</p>
            </div>
            @if($voted)
                <div class="alert alert-success">
                    <p>Du hast bereits für diese Charts abgestimmt! Hier siehst du die ergebnisse für deine Votes</p>
