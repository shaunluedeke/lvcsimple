...
@section('content')
    ...
    <div>
        ...
        <br>
        @auth
            <a class="btn {{(array_key_exists(Auth::user()->userID,$song->getLikes())) ? 'btn-success': 'btn-primary'}} "
               href="{{route('song.updatethums',['song'=>$song->id,'action'=>'up'])}}">Likes: {{count($song->getLikes())}}</a>

            <a class="btn {{(array_key_exists(Auth::user()->userID,$song->getDislikes())) ? 'btn-danger': 'btn-secondary'}} "
               href="{{route('song.updatethums',['song'=>$song->id,'action'=>'down'])}}">Dislikes: {{count($song->getDislikes())}}</a>
        @endauth
        @guest
                    ...
                    <div class="col-md-8">
                        <div class="card">
                            @if(isset($comment['user_id']))
                                ...
                                <div class="card-header">Von {{\App\Models\User::find($comment['user_id'])->username}}
                                    um {{date('H:i',strtotime($comment['time'])). " am ".date('d.m.Y',strtotime($comment['time']))}}</div>
                            @else
                                <div class="card-header">Von {{$comment['name']}}
                                    um {{date('H:i',strtotime($comment['time'])). " am ".date('d.m.Y',strtotime($comment['time']))}}</div>
                            @endif
